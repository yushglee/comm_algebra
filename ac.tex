...
Let $P$ be a property of ring maps,  $P$ is local if
\begin{enumerate}[label=(\alph*)]
...
Let $f\colon X\to S$ be a morphism of schemes,
 $f$ is locally of type  $P$ if for any  $x\in X$
 there is  $x\in U$ to  $f(U)\subset V$ affines
 such that  $P(O_S(V)\to O_X(U))$.
  \begin{lem}
	  If $f(U)\to V$ and $f$ is locally of type $P$
	  for  $P$ local, then
	  $P(O _S(V)\to O_X(U))$.
 \end{lem}
 Use the fact:
 let $X$ be a scheme and  $U,V$ are affines,
 let  $x\in U\cap V$
 ther exists open  $W$ which is standard in both  $U,V$
